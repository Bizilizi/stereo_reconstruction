...
cv::Mat LinearSearch::computeDisparityMap(double smoothFactor) {
    ...
    cv::Mat dispMap = cv::Mat::zeros(h2, w2, CV_64F);
    int max = 0;
    for (int i = 0; i < h2; i++) {
        for (int j = 0; j < w2; j++) {

            if (leftImage.at<cv::Vec3b>(i, j) == cv::Vec3b{0, 0, 0}) {                // skip black pixels which are at borders
                dispMap.at<double>(i, j) = 0;
                continue;
            }

            int col = 0;
            cv::Vec3b pixel_ij = rightImage.at<cv::Vec3b>(i, j);
            double min = std::numeric_limits<double>::max();;
            for (int k = j; k < j+200; k++) {
                if (k < 0)
                    continue;
                cv::Vec3b pixel_ik = leftImage.at<cv::Vec3b>(i, k);
            ...
            if (col - j < 0)
                std::cout << "negative\n";
            dispMap.at<double>(i, j) = static_cast<double>(col - j);
        }
    }
    //cv::Mat dispImg;
